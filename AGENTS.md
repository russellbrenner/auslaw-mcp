...
| `AUSTLII_USER_AGENT`    | No                | User-Agent string for AustLII requests. Must match the browser session that captured `AUSTLII_COOKIE`, or Cloudflare invalidates the cookie.                                                       |
| `AUSTLII_TIMEOUT`       | No                | AustLII request timeout in ms                                                                                                                                                                      |
| `AUSTLII_COOKIE`        | For AustLII (most of the time) | Cookie header value (e.g. `cf_clearance=...; __cf_bm=...`) captured from a real browser session, bypassing Cloudflare's bot challenge. Without it, AustLII calls 403 and surface a re-extraction reminder. See README. |
